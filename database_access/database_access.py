...
# класс пользователя для создания нескольких подключений со своим пользователем
class User:
    def __init__(self, ip, level, password):
        self.ip = ip  # ip-адрес
        self.level = level  # уровень доступа
        ...
        # генерация массива баз данных текущего пользователя
...
class Server:
    ...
    # функция авторизации
    def auth(self, user, addr, this_user=None):
        ...
        if not (user_ip in self.blocked):  # если пользователь не заблокирован
            ...
            if this_user == None:  # если пользователь не найден, создание нового
                self.users.append(User(addr[0], 'low level user', 'root'))
                self.save()
                this_user = self.get_user(addr[0])

            if this_user.password != '':  # запрос пароля
    ...
    # функция бесконечного принятия подключений
    def start_server(self):
        while True:
            user, addr = self.ser.accept()
            Thread(target=self.auth, args=(user, addr,)).start()  # вызов потока авторизации

    # функция обработки всех сообщений от пользователя
